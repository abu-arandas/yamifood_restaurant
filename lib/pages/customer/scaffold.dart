...
class _CustomerScaffoldState extends State<CustomerScaf> {
  ...
  void initState() {
    ...
    scrollController.addListener(
      () => setState(() => scrolled = scrollController.offset >= screenHeight(context) * 0.25),
    );
  }
  ...
  Widget build(BuildContext context) => StreamBuilder<User?>(
        ...
        builder: (context, authSnapshot) {
          List navBarItems() => [
                ...
                if (authSnapshot.hasData) link(text: 'Profile', function: () => page(const UserProfile())),
                if (authSnapshot.hasData) link(text: 'Orders', function: () => page(const OrdersPage())),
                if (!authSnapshot.hasData) link(text: 'Sign In', function: () => page(const SignIn())),
                link(
                  text: 'Contact Us',
                  function: () => page(const CustomerScaf(pageName: 'Contact Us', body: ContactUs())),
                ),
                if (authSnapshot.hasData) link(text: 'sign Out', function: () => UserServices.instance.signOut()),
              ];
                    ...
                    if (widget.pageName != 'Home')
                      Container(
                        ...
                        child: Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.center,
                          children: [
                            Text(widget.pageName.toUpperCase(), style: title(context: context, color: primary)),
                            Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              mainAxisSize: MainAxisSize.max,
                              crossAxisAlignment: CrossAxisAlignment.center,
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      link(
                                        ...
                                        function: () async => await launchUrl(Uri.parse('tel:${App.phone.international}')),
                                      ),
                                      link(
                                        text: App.email,
                            ...
                            child: Wrap(
                              ...
                              children: [
                                ...
                                TextButton(
                                  ...
                                  style: TextButton.styleFrom(padding: EdgeInsets.symmetric(vertical: 0, horizontal: dPadding / 2)),
                                  child: Text(App.name),
                                ),
                                const Text(
                                  'Designed by',
                                ...
                                TextButton(
                                  onPressed: () async => await launchUrl(Uri.parse('https://ehab-arandas.web.app/')),
                                  style: TextButton.styleFrom(padding: EdgeInsets.symmetric(vertical: 0, horizontal: dPadding / 2)),
  ...
  Widget link({required String text, required void Function()? function}) => TextButton(
        onPressed: function,
        style: ButtonStyle(
          foregroundColor: MaterialStateProperty.resolveWith((states) => states.contains(MaterialState.hovered) ||
                  states.contains(MaterialState.pressed) ||
                  states.contains(MaterialState.focused) ||
                  widget.pageName == text
              ? primary
              : white),
          textStyle: MaterialStateProperty.resolveWith((states) => states.contains(MaterialState.hovered) ||
                  states.contains(MaterialState.pressed) ||
                  states.contains(MaterialState.focused) ||
                  widget.pageName == text
