  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        //
        List<BootstrapCol> newChildren = List.from(children);
        newChildren.sort((a, b) => (a.orderPerSize[pfx] ?? 0) - (b.orderPerSize[pfx] ?? 0));
...
class BootstrapVisibility extends StatelessWidget {
  ...
  void _initialize() {
    ...
    List<String> parts = sizes.isEmpty
        ? []
        : sizes.toLowerCase().split(' ').where((t) => t.trim().isNotEmpty).toList();
    for (var part in parts) {
      for (var pfx in _prefixes) {
